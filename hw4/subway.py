...
def subway(**lines):
    ...
    for line in lines:
        ...
        for i in range(len(stations)):
            ...
            stationB  = []
            stationA = []
            if i >0: stationB = stations[i-1]
            if i <= len(stations)-2: stationA = stations[i+1]
            if station not in dicc:
                dicc[station] = {}
            if stationA and stationB:
                dicc[station][stationA] = line
                dicc[station][stationB] = line
            elif stationA:
                dicc[station][stationA] = line
            elif stationB:
                dicc[station][stationB] = line
...
def longest_ride(system):
    ...
    for s in system:
        lStations.append(s)
    allPaths = combinations(lStations,2)
    for path in allPaths:
        here = path[0]
        there = path[1]
        temp = ride(here,there,system)
        if len(temp)>len(maxPath):
            maxPath = temp
    return maxPath
...
def test_ride():
    #print boston
    #for station in  boston:
    #    print 'station:'+str(station)+':'+str(boston[station])
